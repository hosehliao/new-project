    ...
    _initMediaSwitch: function() {
      ...
      $(this.selectors.productThumbImages)
        .on('click', function(evt) {
          ...
          var dataMediaID = $el.data('id');
          var productJson = JSON.parse($('#ProductJson-product-template').html());
          $(productJson.variants).each(function(index,element){
            if(mediaId == dataMediaID) {
              $('[data-index="option2"]').val(element.option2).trigger('change');
            }
          });
...
$(document).ready(function() {
  ...
  sections.register('logobar', theme.logobar);
